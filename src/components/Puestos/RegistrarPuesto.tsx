...
const RegistrarPuesto: React.FC<AgregarProps> = ({
  open,
  handleClose,
  puesto,
}) => {
    ...
    const fetchBloques = async () => {
      try {
        const response = await axios.get(
          "https://mercadolasestrellas.online/intranet/public/v1/blocks"
        );
  ...
  useEffect(() => {
    const fechGiroNegocio = async () => {
      try {
        const response = await axios.get(
          "https://mercadolasestrellas.online/intranet/public/v1/giro-negocios"
        );
  ...
  useEffect(() => {
    const fetchPuestos = async () => {
      try {
        const response = await axios.get(
          "https://mercadolasestrellas.online/intranet/public/v1/puestos/select"
        ); // publico
  ...
  useEffect(() => {
    if (bloqueSeleccionado) {
      const puestosFiltrados = puestos.filter(
        (puesto) => puesto.id_block === bloqueSeleccionado
      );
  ...
  useEffect(() => {
    const fetchSocios = async () => {
      try {
        const response = await axios.get(
          "https://mercadolasestrellas.online/intranet/public/v1/socios"
        ); // publico
  ...
  const registrarPuesto = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await axios.post(
        "https://mercadolasestrellas.online/intranet/public/v1/puestos",
        dataToSend
      );
      if (response.status === 200) {
        const mensaje =
          response.data.message || "Puestoss registrado con éxito";
  ...
  const editarPuesto = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://mercadolasestrellas.online/intranet/public/v1/puestos/${puesto?.id_puesto}`,
        dataToSend
      );
      if (response.status === 200) {
  ...
  const asignarPuestoSocio = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await axios.post(
        "https://mercadolasestrellas.online/intranet/public/v1/puestos/asignar",
        dataToSend
      );
      if (response.status === 200) {
  ...
  const asignarInquilino = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await axios.post(
        "https://mercadolasestrellas.online/intranet/public/v1/inquilinos",
        dataToSend
      ); // Publico

      if (response.status === 200) {
        const mensaje =
          response.data.message || "E l inquilino se registró correctamente";
        mostrarAlerta("Registro exitoso", mensaje, "success").then(() => {
          onRegistrar();
          handleCloseModal();
        });
  ...
  const registrarGiroNegocio = async (
    ...
    try {
      ...
      if (response.status === 200) {
        const mensaje =
          response.data || "El giro de negocio se registró correctamente";
        mostrarAlerta("Registro exitoso", mensaje, "success").then(() => {
          onRegistrar();
          handleCloseModal();
        });
              ...
              <Grid item xs={12} sm={6}>
                ...
                <FormControl fullWidth required>
                  <Autocomplete
                    ...
                    isOptionEqualToValue={(option, value) =>
                      option.id_socio === Number(value.id_socio)
      ...
      botones={
        <BotonesModal
          ...
          action={async (e) => {
            ...
            if (activeTab === 1) {
              result = await mostrarAlertaConfirmacion(
                // Mostrar alerta de confirmación para asignar puesto
            ...
            if (activeTab === 2) {
              result = await mostrarAlertaConfirmacion(
                // Mostrar alerta de confirmación para asignar inquilino
            ...
            if (activeTab === 3) {
              result = await mostrarAlertaConfirmacion(
                // Mostrar alerta de confirmación para registrar bloque
            ...
            if (activeTab === 4) {
              result = await mostrarAlertaConfirmacion(
                // Mostrar alerta de confirmación para registrar giro de negocio
