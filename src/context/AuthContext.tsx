...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [usuario, setUsuario] = useState<string | null>(null); // Estado para almacenar el nombre de usuario
  ...
  const login = (nombreUsuario: string) => {
    ...
    localStorage.setItem('autenticado', JSON.stringify(true));
    localStorage.setItem('usuario', nombreUsuario); // Guarda el nombre de usuario en el localStorage
  }
  ...
  const logout = () => {
    ...
    localStorage.removeItem('autenticado');
    localStorage.removeItem('usuario'); // Elimina el nombre de usuario del localStorage
  }

  useEffect(() => {
    const manejarAutenticacion = () => {
      const guardarAutenticado = localStorage.getItem('autenticado');
      const guardarUsuario = localStorage.getItem('usuario'); // Obtiene el nombre de usuario
      if (guardarAutenticado) {
        setAutenticado(JSON.parse(guardarAutenticado));
        setUsuario(guardarUsuario); // Actualiza el nombre de usuario
      }
    };
